...
exports.updateProfile = async (req,res) =>{
    try {
        ...
        const updatedProfile = await Profile.findByIdAndUpdate(profileId, {dateOfBirth, gender, about, contactNumber}, {returnDocument:"after"});
...
exports.updateDisplayPicture = async (req, res) => {
    try {
      ...
      return res.status(500).json({
        success: false,
        message: error?.message || error?.error?.message || "Could not update display picture",
      })
    }
};
...
//         const id=req.user.id;
//         //validation
//         if(!contactNumber || !gender) return res.status(400).json({success:false,message:"All fields are required"});

//         //find profile and update
//         const userDetails=await User.findById(id);
//         const profileId=userDetails.additionalDetails;

//         if(!profileId) return res.status(400).json({success:false,message:"Profile does not exist"});
